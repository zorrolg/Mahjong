...
public class FinishBuildListener implements IEventListener
{
     ...
     * @param
     */
    public FinishBuildListener(IEventSource gamePlayerEvent,UserTaskInfo userTask, GamePlayer gamePlayer)
    {
	...
	public void onEvent(int para1, int para2) {
		...
		if(para1 == userTaskInfo.getTaskInfo().getTaskPara1())
		{
			...
			if (para2 >= userTaskInfo.getTaskInfo().getTaskPara2()) {// 如果加上这次操作数值还不够完成数量就更新+1并且return
		    	...
		    	gamePlayerEvent.removeListener(TaskConditionType.FinishBuild, this);
