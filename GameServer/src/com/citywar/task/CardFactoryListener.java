...
public class CardFactoryListener implements IEventListener
{
     ...
     * @param
     */
    public CardFactoryListener(IEventSource gamePlayerEvent,UserTaskInfo userTask, GamePlayer gamePlayer)
    {
	...
	public void onEvent(int para1, int para2) {
		...
		if(para1 == userTaskInfo.getTaskInfo().getTaskPara1() || userTaskInfo.getTaskInfo().getTaskPara1() == 0)
		{
			...
			if(finishNum > userTaskInfo.getTaskFinishNum() || userTaskInfo.getTaskInfo().getTaskPara2() == 0)
			{
				...
				if (finishNum >= userTaskInfo.getTaskInfo().getTaskNum()) {// 如果加上这次操作数值还不够完成数量就更新+1并且return
			    	...
			    	gamePlayerEvent.removeListener(TaskConditionType.CardFactory, this);	
