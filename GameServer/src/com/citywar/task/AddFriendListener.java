...
public class AddFriendListener implements IEventListener
{
     ...
     * @param
     */
    public AddFriendListener(IEventSource gamePlayerEvent,UserTaskInfo userTask, GamePlayer gamePlayer)
    {
	...
	public void onEvent(int para1, int para2) {
		...
		int friendsCount = gamePlayer.getFriends().size();
		
		if( friendsCount != userTaskInfo.getTaskFinishNum())
		{
			//统一更新用户数值
			userTaskInfo.setTaskFinishNum(friendsCount > userTaskInfo.getTaskInfo().getTaskNum() ? userTaskInfo.getTaskInfo().getTaskNum() : friendsCount);
			userTaskInfo.setOp(Option.UPDATE);
			
			
			if (friendsCount >= userTaskInfo.getTaskInfo().getTaskNum()) {// 如果加上这次操作数值还不够完成数量就更新+1并且return
				
				userTaskInfo.setTaskStatus(TaskMgr.TASK_STATUS_ALREADY_FINISH);
		    	gamePlayerEvent.removeListener(TaskConditionType.AddFriend, this);
