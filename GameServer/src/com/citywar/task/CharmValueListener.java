...
public class CharmValueListener implements IEventListener
{
     ...
     * @param
     */
    public CharmValueListener(IEventSource gamePlayerEvent,UserTaskInfo userTask, GamePlayer gamePlayer)
    {
	...
	public void onEvent(int para1, int para2) {
		...
		int finishNum = gamePlayer.getPlayerInfo().getCharmValve();
		...
		if(finishNum > userTaskInfo.getTaskFinishNum())
		{
			...
			if (finishNum >= userTaskInfo.getTaskInfo().getTaskNum()) {// 如果加上这次操作数值还不够完成数量就更新+1并且return
		    	...
		    	gamePlayerEvent.removeListener(TaskConditionType.CharmValue, this);
