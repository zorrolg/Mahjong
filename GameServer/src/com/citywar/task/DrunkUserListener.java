...
public class DrunkUserListener implements IEventListener
{
     ...
     * @param
     */
    public DrunkUserListener(IEventSource gamePlayerEvent,UserTaskInfo userTask, GamePlayer gamePlayer)
    {
	...
	public void onEvent(int para1, int para2) {
		...
		int finishNum = userTaskInfo.getTaskFinishNum() + 1; //gamePlayer.getPlayerReference().getCurrentSlavesCount();//当前完成的次数
		...
		if (finishNum >= userTaskInfo.getTaskInfo().getTaskNum()) {// 如果加上这次操作数值还不够完成数量就更新+1并且return
	    	...
	    	gamePlayerEvent.removeListener(TaskConditionType.DrunkUser, this);
