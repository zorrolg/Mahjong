...
public class Player {
	...
	public boolean PlayerTimeOutCall() {
		...
		// if the call is the first,call current player size +1,else current
		// numbers add 1.
		// if the call is the first,randomly create a point,else the same to
		// last.
		...
		// PlayerTimeOutCall========================================================="
		// + diceGame.getDiceNumber() + "====" + diceGame.getDicePoint() +
		// "====" +diceGame.getHasCallOne());
		return false;
	}
	...
	public void nextCallWhenHave() {
		...
		if (rndtype < robotLevelInfo.getRealcallmostpercent()) {
			...
			// 1================"+rndtype + "=================================="
			// + robotLevelInfo.getRealcallmostpercent()
			...
			// System.out.println("================================real type
			// 3================"+rndtype + "=================================="
			// + robotLevelInfo.getRealcallrndpercent()
			// + "======" + robotLevelInfo.getRealcallnumberpercent());
			// 获取可叫的点数
		...
		// 叫上一个人的数字,个数+1OR+2: 概率70%
			...
			// System.out.println("================================real type
			// 2================"+rndtype + "=================================="
			// + robotLevelInfo.getRealcalladdpercent()
			// + "======" + robotLevelInfo.getRealcallnumberpercent());

			int finallyNumber = 0;
			if (random.next(1, 101) < robotLevelInfo.getUnRealcallnumberpercent())
				finallyNumber = diceGame.getDiceNumber() + 1 > totalDiceSize ? totalDiceSize
						: diceGame.getDiceNumber() + 1;
			else
				finallyNumber = diceGame.getDiceNumber() + 2 > totalDiceSize ? totalDiceSize
						: diceGame.getDiceNumber() + 2;
			diceGame.setDiceNumber((byte) finallyNumber);

			// diceGame.setDiceNumber((byte) (diceGame.getDiceNumber() + 1 >
			// totalDiceSize ? totalDiceSize
			// : diceGame.getDiceNumber() + 1));
		}
	...
	public void nextCallWhenNotHave() {
		...
		if (random.next(1, 101) < robotLevelInfo.getUnRealcallmostpercent()) {
			// System.out.println("================================un real type
			// 1================"+rndtype + "=================================="
			// + robotLevelInfo.getUnRealcallmostpercent()
			...
			diceGame.setDiceNumber((byte) finallyNumber);
			diceGame.setDicePoint((byte) randomDicePoint);
		} else if (random.next(1, 101) < robotLevelInfo.getUnRealcallmostpercent()
				+ robotLevelInfo.getUnRealcallrndpercent()) {
			// System.out.println("================================un real type
			// 3================"+rndtype + "=================================="
			// + robotLevelInfo.getUnRealcallrndpercent()
			// + "======" + robotLevelInfo.getUnRealcallnumberpercent());
		...
		// 叫上一个人的数字,个数+1OR+2: 概率20%
			...
			// System.out.println("================================un real type
			// 2================"+rndtype + "=================================="
			// + robotLevelInfo.getUnRealcalladdpercent()
			// + "======" + robotLevelInfo.getUnRealcallnumberpercent());
	...
	public void robotTurn() {
		...
		// System.out.println("机器人叫的时候diceGame.getTurnQueue()的大小为==" +
		// diceGame.getTurnQueue().size());
		...
		// System.out.println("");
		// System.out.println("robotTurn================" + currentNumber +
		// "====" + total);
		...
		if (isLowArea) {
			...
			// System.out.println("not real open================"+rndOpen +
			// "======" + robotLevelInfo.getUnRealOpenPercent());
			...
			int rndOpen = random.next(1, 101);
			// System.out.println("real open================"+rndOpen + "======"
			// + robotLevelInfo.getRealOpenPercent());
	...
	public boolean isHaveCurrentPoint(DiceGame diceGame) {
		return diceGame.haveCurrentPointCount(this.getDiceNumbers(), diceGame.getDicePoint()) > 0;
	}
	...
	public int getLowestCanCallNum(DiceGame diceGame, int randomDicePoint, boolean isCallOne) {
		...
		/**
		 * 筛子大小顺序 1 > 6 > 5 > 4 > 3 > 2 最小可叫值的规则: 如果随机获得的筛子大于等于场上的数字，最小值为场上个数
	...
	public String toString() {
		String playerString = String.format("PlayerID: %d,PlayerState: %d,RoomId: %d,Coins: %d,", getPlayerID(),
				getPlayerState(),
				getPlayerDetail().getCurrentRoom() != null ? getPlayerDetail().getCurrentRoom().getRoomId() : -1,
				getPlayerDetail().getPlayerInfo().getCoins());
		return playerString + "IsPlaying: " + getPlayerDetail().getIsPlaying() + "IsRobot: "
				+ getPlayerDetail().getIsRobot() + "UserName: " + getPlayerDetail().getPlayerInfo().getUserName();
	}
