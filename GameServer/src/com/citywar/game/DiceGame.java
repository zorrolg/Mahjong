...
//                gp.addGpDirect(-gp.getPlayerInfo().getLevel() * 5);
                ...
                player.getPlayerDetail().getOut().sendUpdatePrivateInfo(player.getPlayerDetail().getPlayerInfo(), (byte)1);
                this.sendToAll(pkg);
                ClearWaitTimer();
        	}
        } 
     ...
     * @param hallType
     */
    public DiceGame(int gameId, int hallId, BaseRoom baseRoom, List<GamePlayer> list,
			 int gameType, int timeType, HallTypeInfo hallType) {
        super(gameId, hallId, baseRoom, gameType, timeType, hallType, 
    ...
    public void gameOver(boolean isGrab)
    {
        	...
        	)
        {
            ...
            try
            {
                ...
                // 被叫玩家ID
                packet.putInt(calledPlayer == null ? -1
                        : calledPlayer.getPlayerDetail().getUserId());
...
//    					 + drunkCut +  "======");
    			...
    			if(hallGameType == HallGameType.CONTEST)
    			{
        			...
        			float levelPara = (float)(successPlayer.getPlayerInfo().getLevel() - failPlayer.getPlayerInfo().getLevel())/20;
...
//                    winCharmValve = 20 * ( 1-  (successPlayer.getPlayerInfo().getLevel() - failPlayer.getPlayerInfo().getLevel())/10);
                    ...
                    movieDelay += 2000;
                    //System.out.println("winCharmValve=====2===" + CoinPara + "======" + levelPara + "======" + drunkPara +  "======" + winCharmValveFloat +  "======" + winCharmValve +  "======");
...
//            	System.out.println("drunkCutdrunkCut========" + drunkCut );
                ...
                if (failPlayer.isFullDrunk()) {//如果输者喝醉了，更新奴隶关系，现在都可以
                	...
                	String strMsg = LanguageMgr.getTranslation("CityWar.Trumpet.Drunk",successPlayer.getPlayerInfo().getUserName(),failPlayer.getPlayerInfo().getUserName());		
...
//                    	String strNotify = LanguageMgr.getTranslation("CityWar.Ref.Notify", successPlayer.getPlayerInfo().getUserName(), work.getWorkName());	                    	
                    	failPlayer.getOut().sendPlayerDrunkNotify(work.getWorkTime()*3600, money, work.getWorkName(), successPlayer.getPlayerInfo().getUserName());
                    	workType = work.getId(); 
                    ...
                    {
                    	failPlayer.getPlayerInfo().setDrunkLevelContest(failPlayer.getPlayerInfo().getDrunkLevelLimit());
                    }
...
//                        }
                        ...
                        if(winCharmValve > 0)
                        	UserTopMgr.checkLevelTop(true, gamePlayer, this.hallType, gamePlayer.getCurrentStage(), winCharmValve);
...
//                    	gamePlayer.addGpDirect(GameDataType.FAIL_ADD_GP);//失败20
                    ...
                    else if (tempPlayer.getPlayerID() == failID)
                    {
                    	...
                    	if(winCharmValve > 0)
                    		UserTopMgr.checkLevelTop(false, gamePlayer, this.hallType, gamePlayer.getCurrentStage(), -winCharmValve);
...
//                    	gamePlayer.addCoins(dogfallMoney * -1);//不胜不负：45
                    ...
                    // 道具数量, 必须重构
                    packet.putInt(gamePlayer.getPropBag().getPropCountByType(
                                                                       PropType.WAKE_TYPE));
...
//                	gamePlayer.getOut().sendCardUsedList(tempPlayer.getPlayerDetail().getListGameCard());
            ...
            gameState = GameState.GameOver;
            GameTotal += 1;
            startGameDelay = (this.getTurnQueue().size() * 1.6 + hitDiceNumber.floatValue() * 0.3 + 7.5) * 1000.0 + movieDelay;
...
//    }
    ...
    public boolean canGameOver()
    {
        ...
        if (this.getTurnQueue().size() < 2 
        		|| turnIndex >= 20
                || DiceNumber >= ((this.getTurnQueue().size() * 5) + 1))
        {
...
//    	System.out.println("nextTurn================================================================================================" + this.DiceNumber);
        ...
        {
            if (!isPlayerCall && currentTurnPlayer != null
                    && !currentTurnPlayer.getPlayerDetail().getIsRobot())
            {
            ...
            if (currentTurnPlayer.getPlayerDetail().getIsRobot() || currentTurnPlayer.getPlayerDetail().getIsRobotState())
            {
...
//        System.out.println("gamer sendGameNextTurn=========================================================" + getDiceNumber() + "====" + getDicePoint() + "====" + getHasCallOne());
    }
    ...
    /**
     * create dice points randomly,and send to client, at the same time change
     * game state to Prepared.
     */
    @Override
    ...
    /**
     * if the game has only one player or the game is playing,return;
     * otherwise,checking player's state,sending true message that means game
     * can start when all player had shaked cellphone ,in other side,sending
     * false message.
     */
    @Override
    public void sendShakePrepare()
    {
        ...
        if ((getGameState() == GameState.Created || getGameState() == GameState.Prepared)
                && list.size() > 1)
        {
