				...
				if (client != null) {
					...
					try {
						...
						if ("GET".equalsIgnoreCase(method)) {
							...
							} else if (tempGetMap.containsKey("secret")
									&& tempGetMap.get("secret").equals("15019225062")) // 同步推
							{
							...
							while (postStr != null) {
								...
								if ("".equals(postStr)) {
									...
									contentLength = Integer
											.parseInt(postStr.substring(postStr.indexOf("Content-Length") + 16));
								}
							}
							// contentLength -= 4;
							...
							for (String str : list) {
								if (str.indexOf("=") != -1) {
									...
									String valve = URLDecoder.decode(str.substring(str.indexOf("=") + 1, str.length()),
											"UTF-8");
							...
							if ((tempMap.containsKey("source")
									&& tempMap.get("source").indexOf(ChannelType.ZHIFUBAO.getValue()) != -1)
									|| (tempMap.containsKey("out_trade_no") && tempMap.get("out_trade_no")
											.indexOf(ChannelType.ZHIFUBAO.getValue()) != -1))// 支付宝-－客户端配置
							{
